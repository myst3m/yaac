...
(defn main-usage [options-summary]
  (->> ["Usage: yaac [options] <action>"
        ...
        ""
        "  # Deploy"
        "  yaac deploy app T1 Production my-app target=ch2:rootps v-cores=0.1 \\"
        "    -g T1 -a my-app -v 1.0.0 +mule.env=ch2"
        ""
        "  # A2A — Agent-to-Agent protocol"
        "  yaac a2a init T1 Sandbox my-broker         # connect via Anypoint app"
        "  yaac a2a init https://agent.example.com    # connect via direct URL"
        "  yaac a2a send 'Hello, what can you do?'    # send to last session"
        "  yaac a2a console my-broker                  # interactive multi-turn"
        "  yaac a2a console                            # resume last console"
        ""
        ...
        ""
        "  # Mule connector schema / validator"
        "  yaac connector collect                      # populate ~/.yaac/schema from ~/.m2"
        "  yaac connector list"
        ...
        "  yaac otel 4318                              # OTLP collector on :4318"
        ""
