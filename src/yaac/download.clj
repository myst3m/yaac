...
(defn usage [summary-options]
  (->> ["Usage: download <resources> [options]"
        ...
        "Examples:"
        ""
        "# Download API proxy from current org/env"
        "  > yaac download proxy hello-api"
        ""
        "# Download API proxy with explicit org and env"
        "  > yaac download proxy T1 Production hello-api"
        ""
        "# Using 'api' resource (same as 'proxy')"
        "  > yaac download api T1 Production hello-api"
        ""
        ""]
    (str/join \newline)))
