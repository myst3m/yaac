...
(defn usage [summary-options]
  (->> ["Usage: deploy manifest <manifest.yaml> [options]"
        ...
        "Useful for deploying entire API ecosystems (System/Process/Experience APIs)."
        ""
        "Options:"
        ""
        summary-options
        ...
        "    2. src/main/resources/config/config.properties"
        "    3. src/main/resources/config.yaml"
        "    4. src/main/resources/config.properties"
        "    5. --config-properties FILE (relative to each app dir)"
        ""
        "  Examples:"
        "    yaac deploy manifest --scan                    # Scan current dir"
        "    yaac deploy manifest --scan ./system-apis      # Scan specific dir"
        "    yaac deploy manifest --scan . --config-properties src/main/resources/env/local.yaml"
        ""
        "Manifest YAML Schema:"
        ""
        "  organization: T1                    # Business group name"
        "  environment: Production             # Environment name"
        ""
        "  # Target definitions (key = actual target name from 'yaac get rtt')"
        "  targets:"
        "    cloudhub-ap-northeast-1:          # CloudHub 2.0"
        "      type: ch2"
        "      instance-type: small            # nano | small | small.mem"
        ""
        "    my-rtf-cluster:                   # Runtime Fabric"
        "      type: rtf"
        "      cpu: [500m, 1000m]              # [reserved, limit]"
        "      mem: [1200Mi, 1200Mi]"
        ""
        "    leibniz:                          # Hybrid on-premise"
        "      type: hybrid"
        ""
        "  # Application definitions"
        "  apps:"
        "    - name: customer-sapi             # Deployed app name"
        "      asset: T1:customer-sapi:1.0.0   # group:artifact:version"
        "      target: my-rtf-cluster          # Target name from above"
        ...
        ""
        "Target Types:"
        "  rtf     - Runtime Fabric (cpu, mem options)"
        "  ch2     - CloudHub 2.0 (instance-type, v-cores options)"
        "  hybrid  - On-premise Hybrid server"
        ""
        "connects-to Feature:"
        "  Automatically generates <app-name>.url properties:"
        "  - RTF:    http://<app>.svc.cluster.local:<port>"
        "  - CH2:    https://<app>.cloudhub.io"
